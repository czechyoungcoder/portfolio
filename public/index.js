...
headerNav.addEventListener("click", (e) => {
  if (
    ...
    (e.target.classList.contains("header__link") && isMenuOpen())
  )
    toggleNav();
});
...
const sendMail = (mail) => {
  fetch("http://localhost:3000/send", {
    ...
    .then((response) => {
      if (language === "cs")
        submitMessage.textContent = "Zpráva byla úspěšně doručena!";
    ...
    .catch((err) => {
      if (language === "cs")
        submitMessage.textContent =
          "Zprávu nebylo možné odeslat... Zkus to později.";
      else
        submitMessage.textContent =
          "An error occured while sending the message... Try it again later.";
    });
};
